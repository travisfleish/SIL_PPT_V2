#!/usr/bin/env python3
"""
Crop Shake Shack logo to circle, removing black border
"""
...
def remove_border_and_crop_to_circle(img_path, output_path, size=400, border_threshold=10):
    """Remove black border and crop to circle"""
    ...
    if img_array.shape[2] == 4:
        rgb = img_array[:, :, :3]
        # Check if any RGB channel is above threshold (not black)
...
def main():
    """Crop Shake Shack logo"""
    logo_dir = Path(__file__).parent / "assets" / "logos" / "merchants"
    logo_path = logo_dir / "shake_shack.png"
    output_path = logo_dir / "shake_shack.png"
    
    if not logo_path.exists():
        print(f"⚠️  File not found: shake_shack.png")
        return
    
    print("🎨 Cropping Shake Shack logo to circle (removing black border)...\n")
    
    try:
        remove_border_and_crop_to_circle(logo_path, output_path, size=400)
        print("\n✨ Done!")
    except Exception as e:
        print(f"❌ Error processing shake_shack.png: {e}")
        import traceback
        traceback.print_exc()
